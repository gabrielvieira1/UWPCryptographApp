...
namespace UWPCryptographApp
{
  public class Library
  {
    ...
    private static CryptographicKey key = algorithm.CreateSymmetricKey(CryptographicBuffer.CreateFromByteArray(values));
    ...
    public void Encrypt(TextBox display)
    {
      try
      {
        IBuffer buffer = CryptographicBuffer.ConvertStringToBinary(display.Text, BinaryStringEncoding.Utf8);
    ...
    public void Decrypt(TextBox display)
    {
      try
      {
        ...
        display.Text = CryptographicBuffer.ConvertBinaryToString(BinaryStringEncoding.Utf8, bufferDecrypt);
      }
      catch
      {
